...
\begin{figure}[H]
    ...
    \caption{压气机叶栅正常工况流场细节}
    \label{fig:comp_normal2}
\end{figure}

\textbf{正常工况（图~\ref{fig:comp_normal}、图~\ref{fig:comp_normal2}）：} 在设计冲角下，气流顺畅地流过压气机叶栅通道，附面层（边界层）紧贴叶片吸力面与压力面，未发生明显的流动分离。通道内存在连续且均匀的逆压梯度，实现气流动能向压力能的有效转化。各叶片通道内流场均匀一致。
...
\textbf{说明：} 在本次实验提供的图形素材中，目前缺乏直接展示涡轮（高/低压涡轮）内部叶栅流场和气流膨胀加速过程的 CFD 云图。如果实验台或软件中有这一部分的展示，应重点观察并记录：高温高压燃气在涡轮导向器的收缩通道内发生剧烈的膨胀加速，压力与温度骤降，速度转化为动能；随后高速气流冲击转子叶片继续膨胀作功。同时需注意观察叶尖间隙处的泄漏流（Leakage flow）以及通道内的复杂二次流分布。
