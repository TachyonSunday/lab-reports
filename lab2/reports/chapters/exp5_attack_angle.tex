...
实验在工作频率 45 Hz 下进行，四个攻角的进口总压 $P_1^*$ 和进口静压 $P_1$ 同一攻角下共用。$\beta_{\text{in}}$ 为转盘刻度，$\beta_{\text{out}}$ 为三孔探针对准后的探针转台读数。$\beta_1 = 25° + \beta_{\text{in}}$ 为进口气流角，$\beta_2 = \beta_{\text{out}}$ 为出口气流角。

\input{tables/table_exp5_attack_angle.tex}
...
图~\ref{fig:attack} 展示了 $-10\textdegree$ 至 $+5\textdegree$ 攻角范围内的转折角和损失系数变化。
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item $\Delta\beta$ 在 $-10\textdegree \sim +5\textdegree$ 范围内随攻角增大而增大，反映叶片对气流的偏转能力随来流攻角的增大而增强。
        \item 正攻角下转折角较大，但叶片负荷也相应增大。若攻角继续增大超出实验范围，吸力面附面层将开始分离，转折能力下降。
    \end{itemize}
    \item \textbf{损失系数 $\omega$ 的攻角特性}：
    \begin{itemize}
        \item $\omega$ 在非设计攻角（$-5\textdegree$）附近取得最小值，呈典型的盆形曲线。但实际上损失系数不可能为0，因为叶栅存在不可避免的摩擦损失，所以可能是实验误差或测量误差导致的。
        \item 偏离设计攻角后 $\omega$ 增大。$-10\textdegree$ 时负攻角使压力面可能出现分离；$+5\textdegree$ 时正攻角使吸力面逆压梯度增强。
    \end{itemize}
    \item \textbf{与额定特性的联系}：
    \begin{itemize}
