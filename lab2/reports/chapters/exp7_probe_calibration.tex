...
首先计算孔1和孔3的平均压力：
\begin{equation}
    P_{\text{avg}} = \frac{P_1 + P_3}{2}
\end{equation}

\subsubsection{方向标定系数 $K_\alpha$}

\begin{equation}
    K_\alpha = \frac{P_1 - P_3}{P_2 - P_{\text{avg}}}
\end{equation}

$K_\alpha$ 反映左右两孔的压力差与中心孔动压的相对关系，与迎角 $\alpha$ 近似呈线性关系。$K_\alpha = 0$ 时对应 $\alpha = 0\textdegree$（孔2正对来流）。

\subsubsection{总压标定系数 $K_t$}

\begin{equation}
    K_t = \frac{P_t - P_2}{P_2 - P_{\text{avg}}}
\end{equation}

$K_t$ 反映探针测得的中心孔总压与真实来流总压的偏差。

\subsubsection{速度标定系数 $K_v$}

\begin{equation}
    K_v = \frac{P_t - P_s}{P_2 - P_{\text{avg}}}
\end{equation}
